...
def register_file_system_read_file_tool(server: FastMCP) -> None:
    ...
    async def file_system_read_file(input_data: FileSystemReadFileInput) -> Dict[str, Any]:  # pyright: ignore[reportUnusedFunction]
        ...
        try:
            ...
            if not path.is_file():
                return build_mcp_tool_error_response({
                    "error_message": f"Path is not a file: {input_data.path}"
                })
