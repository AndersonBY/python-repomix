...
class PackRemoteRepositoryInput(BaseModel):
    ...
    remote: str = Field(
        description="GitHub repository URL or user/repo format (e.g., 'yamadashy/repomix', 'https://github.com/user/repo')"
    )
    compress: bool = Field(
        default=False,
    ...
    include_patterns: Optional[str] = Field(
        default=None,
        description="Specify files to include using fast-glob patterns"
    )
...
def register_pack_remote_repository_tool(server: FastMCP) -> None:
    ...
    async def pack_remote_repository(input_data: PackRemoteRepositoryInput) -> Dict[str, Any]:  # pyright: ignore[reportUnusedFunction]
