...
def register_file_system_read_directory_tool(server: FastMCP) -> None:
    ...
    @server.tool(
        ...
        description=(
            "List the contents of a directory using an absolute path. "
            "Returns a formatted list showing files and subdirectories with clear indicators."
        )
    )
    async def file_system_read_directory(input_data: FileSystemReadDirectoryInput) -> Dict[str, Any]:  # pyright: ignore[reportUnusedFunction]
        ...
        try:
            ...
            if not path.is_absolute():
                return build_mcp_tool_error_response({
                    "error_message": f"Path must be absolute: {input_data.path}"
                })
            
            # Check if directory exists
            if not path.exists():
                return build_mcp_tool_error_response({
                    "error_message": f"Directory not found: {input_data.path}"
                })
            
            if not path.is_dir():
                return build_mcp_tool_error_response({
                    "error_message": f"Path is not a directory: {input_data.path}"
                })
            ...
            try:
                ...
                return build_mcp_tool_error_response({
                    "error_message": f"Permission denied: {input_data.path}"
                })
